        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900 lg:col-span-1">
            ...
            <div class="mt-4 max-h-140 space-y-2 overflow-auto">
                @forelse ($lotes as $lote)
                    <button
                        type="button"
                        wire:key="lista-lote-{{ $lote->id }}"
                        wire:click="selectLote({{ $lote->id }})"
                    ...
                    >
                        ...
                        <div class="text-xs text-zinc-600 dark:text-zinc-300">{{ \App\Models\Lote::LABELS[$lote->estado] ?? ucfirst($lote->estado) }}</div>
                    </button>
                @empty
                    <div class="rounded-lg border border-zinc-200 px-3 py-4 text-sm text-zinc-600 dark:border-zinc-700 dark:text-zinc-300">
            ...
            @if ($selectedLote)
                ...
                <div class="mt-4 grid gap-3 sm:grid-cols-2">
                    <div class="rounded-lg border border-zinc-200 p-3 dark:border-zinc-700">
                        ...
                        <div class="text-sm font-semibold text-zinc-900 dark:text-white">{{ \App\Models\Lote::LABELS[$selectedLote->estado] ?? ucfirst($selectedLote->estado) }}</div>
                    </div>
                    ...
                    <div class="rounded-lg border border-zinc-200 p-3 dark:border-zinc-700 sm:col-span-2">
                        ...
                        <div class="text-sm font-semibold text-zinc-900 dark:text-white">{{ $selectedLote->nombre_adquiriente ?: 'No registrado' }}</div>
                    </div>

                    <div class="rounded-lg border border-zinc-200 p-3 dark:border-zinc-700 sm:col-span-2">
                        <div class="text-xs text-zinc-500 dark:text-zinc-400">Observaciones</div>
                        <div class="text-sm text-zinc-900 dark:text-white">{{ $selectedLote->observaciones ?: 'Sin observaciones' }}</div>
                    </div>
                </div>
            @else
                <div class="rounded-lg border border-zinc-200 px-3 py-6 text-sm text-zinc-600 dark:border-zinc-700 dark:text-zinc-300">
